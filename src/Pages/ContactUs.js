...
function BookingModal() {
  ...
  useEffect(() => {
    if (activities.length > 0) {
      const propertyViewingDates = activities["property viewing"]
        .filter((activity) => {
      ...
      setDateOptions(propertyViewingDates);
    }
  }, [activities]);
  ...
  useEffect(() => {
    if (selectedDate) {
      const selectedActivity = activities["property viewing"].find(
        (activity) => activity.date === selectedDate
      );
      if (selectedActivity) {
        setTimeOptions(selectedActivity.freeTime.map((time) => time.time));
      } else {
        setTimeOptions([]);
      }
    } else {
