          ...
          {questions.map((question, index) => (
            <div
              ...
              className={`md:cursor-pointer md:mb-8 ${index < questions.length - 1 ? "pb-2 border-b p-4" : "p-4"
                } ${activeIndex === index ? "rounded-lg border border-[#199976] p-4" : ""}`}
              onClick={() => handleQuestionClick(index)}
            >
              <div className="flex items-center justify-between space-x-2 relative md:h-[4rem]">
                <h1
                  className={`font-generalsans text-base ${activeIndex === index ? "text-[#199976]" : "text-[#262626] hover:text-[#199976]"
                    }`}
                >
                  {question}
                </h1>
                {window.innerWidth >= 768 && activeIndex === index ? (
                  <img src={fa} alt="Forward Icon" className="h-5 ml-2 self-center" />
                ) : null}
              </div>
            </div>
          ))}
